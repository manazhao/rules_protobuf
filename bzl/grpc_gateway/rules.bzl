...
load("//bzl:go/rules.bzl", "PB_COMPILE_DEPS", "GRPC_COMPILE_DEPS")
...
def grpc_gateway_library(
    name,
    pb_gateway = "//bzl/grpc_gateway:pb_gateway",
    langs = ["//bzl/grpc_gateway"],
    protos = [],
    imports = [],
    importmap = {},
    inputs = [],
...
def grpc_gateway_binary(name,
  ...
  print("deps %s" % (deps + [proto_label] + grpc_gateway_deps))
